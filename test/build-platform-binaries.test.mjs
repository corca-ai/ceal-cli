...
test("builds one smoke-checked ceal and cealctl release set", async () => {
...
test("rejects cross-platform builds, version drift, and unsafe replacement", async () => {
	await withTempDir(async (root) => {
		await assert.rejects(
			() => buildCealCliPlatformBinaries({ version: "0.64.0", platform: "linux-x64", outputDir: path.join(root, "cross") }, fakeDeps([])),
...
function fakeDeps(calls) {
	return {
		...
		readContract: () => ({ repository: "corca-ai/ceal-cli", release_version: "0.64.0", protocol: {}, first_proof_matrix: { platform: "linux-arm64" }, publication_blockers: [], non_claims: [] }),
