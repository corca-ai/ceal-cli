...
} from "../scripts/worker-gateway-handoff-archive.mjs";
import {
...
test("resolves only a lock-bound exact Gateway archive through a disposable packet", (context) => {
	...
	let packetDirectory;
	const result = resolveLockedGatewayHandoffArchive({ repoRoot: fixture.repoRoot, archiveFile: fixture.archive }, {
...
test("refuses a changed archive or unsafe archive inventory before the input resolver", (context) => {
	...
	assert.throws(() => resolveLockedGatewayHandoffArchive({ repoRoot: fixture.repoRoot, archiveFile: fixture.archive }, {
	...
	assert.throws(() => resolveLockedGatewayHandoffArchive({ repoRoot: unsafe.repoRoot, archiveFile: unsafe.archive }, {
...
test("worker input facade preserves the reviewed lock trust anchor and maps archive failures", () => {
	...
	const result = resolveWorkerReleaseInputsFromLockedGatewayArchive({ repoRoot: "/tmp/ceal-cli-test", gatewayHandoffArchive: "/tmp/ceal-gateway-handoff-0.65.0.tar.gz" }, {
		consumeArchive: (_options, handlers) => handlers.consume({
			lock,
			rawInputs: { protocolTarball: "/tmp/protocol.tgz" },
			resolution: {
				protocol: { producer: { commit: lock.gateway_commit } },
				non_claims: ["This caller-supplied digest binds exact local input bytes; it does not authenticate who supplied that digest or packet."],
			},
		}),
	});
	...
	assert.equal(result.non_claims.some((entry) => entry.startsWith("This caller-supplied digest")), false);
	assert.throws(() => resolveWorkerReleaseInputsFromLockedGatewayArchive({ repoRoot: "/tmp", gatewayHandoffArchive: "/tmp/archive.tar.gz" }, {
		consumeArchive: () => { throw new WorkerGatewayHandoffArchiveError("gateway_handoff_lock_missing", "missing"); },
...
function hasArchiveCode(code) { return (error) => error instanceof WorkerGatewayHandoffArchiveError && error.code === code; }
function hasInputCode(code) { return (error) => error instanceof WorkerReleaseInputError && error.code === code; }
