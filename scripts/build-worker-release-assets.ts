...
export async function runCli(argv: string[], io: Pick<Console, "log" | "error"> = console) {
	...
	try {
		...
		if (parsed.help || (parsed.mode !== "compose" && parsed.mode !== "merge")) {
			io.log(
				"usage: node scripts/build-worker-release-assets.ts compose --out <absolute-dir> --gateway-handoff-archive <absolute-tar.gz> [--version <semver>] [--platform <current-platform>] [--force] [--json]\n       node scripts/build-worker-release-assets.ts merge --out <absolute-dir> --input <composed-dir> [--input <composed-dir> ...] [--force] [--json]",
			);
