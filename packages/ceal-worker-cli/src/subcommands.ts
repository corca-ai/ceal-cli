...
export const CEAL_SUBCOMMANDS = [
	{
		...
		route: ["status"],
		default: true,
		description: "Inspect the available guide and its registration in every supported agent host.",
		usage: "ceal guide status",
		effect: "read_only",
		evidence: "surface",
		result_schema: "ceal.guide.v1",
		recovery: "Restore the source guide or update to a matching signed release, then retry; do not reinstall based on this status alone.",
	},
	{
		parent: "guide",
		route: ["register", "codex"],
		description: "Link this runtime's available guide into the configured Codex skill directory.",
	...
	},
	{
		...
		description: "Link this runtime's available guide into the configured Claude Code skill directory.",
	...
	},
	{
		...
		notes: [
			...
			"currently authorized target was returned. An incomplete page always carries",
			"'target_catalog.next_cursor'; malformed pages are rejected by the decoder.",
			"Target selectors are capability-specific, and some capabilities do not support",
			"--match at all. A capability input_contract describes",
			"call arguments, not what --match accepts; use it only when catalog navigation",
			"or current Gateway guidance",
			"declares that selector form. A rejected URL returns selector_not_supported with",
			"the declared resolver workflow instead of an empty target list.",
		...
		options: [
			...
			"  --match <selector>      Submit a selector only when catalog navigation declares it.",
	...
	},
	{
		parent: "receipt",
		route: ["show"],
		description: "Read safe Gateway audit evidence with the stored access token.",
		usage: "ceal receipt show <request-ref> [--profile <profile-ref>]",
		effect: "read_only",
		evidence: "surface_or_host_decision",
		result_schema: "ceal.receipt.v1",
		recovery:
			"Use the request reference a 'ceal call' returned; if the stored access token is rejected, run 'ceal session refresh' and retry. While no audited outcome exists for it the Gateway answers 'audit_event_not_found'.",
		options: [
		...
		notes: [
			"Receipt readback never invokes a provider or rotates the stored Gateway session.",
			"If the Gateway rejects the stored access token, run 'ceal session refresh'",
			"explicitly, then retry this route.",
		],
	},
	{
		parent: "acceptance",
		...
		recovery:
			"If the stored access token is rejected, run 'ceal session refresh', then re-run; a build tree is refused because it is not an installed release.",
		notes: [
			"The evidence reads do not invoke a provider or rotate the stored session.",
			"If the Gateway rejects the stored access token, run 'ceal session refresh'",
			"explicitly, then retry this route.",
			"Measures the running binary, so there is no --binary option to substitute.",
			"Performs a live discovery. --request-ref embeds the Gateway-audit readback for a receipt 'ceal call' already produced.",
			"That readback does not establish provider state, and this route never performs a provider call.",
