...
import {
	...
	type CealLeasedConsumerControlResponse,
...
export interface LeasedConsumerControlSessionRuntime {
	...
	readonly requestUnixSocket?: (input: Readonly<{ socketPath: string; path: string; body: string; credential: string }>) => Promise<UnixSocketResponse>;
}
...
async function dispatch(credential: string, frame: Uint8Array, runtime: LeasedConsumerControlSessionRuntime): Promise<Uint8Array> {
	...
	if (response.status !== 200 || !isJsonContentType(response.contentType) || response.bytes.byteLength > CEAL_LEASED_CONSUMER_CONTROL_MAX_FRAME_BYTES)
...
function createProtectedFd4(): Readonly<{ read: () => Promise<Uint8Array>; close: () => Promise<void> }> {
	...
	return Object.freeze({
		...
		close: () => stream.destroyed ? Promise.resolve() : new Promise<void>((resolve) => { stream.once("close", () => resolve()); stream.destroy(); }),
	});
}
...
async function readBoundedStream(stream: AsyncIterable<Uint8Array>, maximum: number, abort: () => void): Promise<Uint8Array> {
	...
	for await (const chunk of stream) {
		if (!(chunk instanceof Uint8Array) || chunk.byteLength > maximum - total) { abort(); throw new Error("input_too_large"); }
...
function postUnixSocket(input: Readonly<{ socketPath: string; path: string; body: string; credential: string }>): Promise<UnixSocketResponse> {
	const body = Buffer.from(input.body, "utf8");
	return new Promise((resolve, reject) => {
		let settled = false;
		const finish = (action: () => void) => { if (!settled) { settled = true; action(); } };
		const request = httpRequest({
			socketPath: input.socketPath, path: input.path, method: "POST",
			...
			response.on("data", (chunk: Buffer) => {
				...
				if (total > CEAL_LEASED_CONSUMER_CONTROL_MAX_FRAME_BYTES) { request.destroy(); finish(() => reject(new Error("response_too_large"))); return; }
				chunks.push(chunk);
			});
...
function assertNoDuplicateJsonKeys(text: string): void {
	...
	const string = () => { const start = index; if (text[index] !== '"') throw new Error("invalid_json"); index += 1; while (index < text.length) { const character = text[index]; if (character === "\\") index += 2; else { index += 1; if (character === '"') return JSON.parse(text.slice(start, index)) as string; } } throw new Error("invalid_json"); };
	const value = (): void => { space(); if (text[index] === "{") { index += 1; const keys = new Set<string>(); space(); if (text[index] === "}") { index += 1; return; } for (;;) { space(); const key = string(); if (keys.has(key)) throw new Error("duplicate_json_key"); keys.add(key); space(); if (text[index++] !== ":") throw new Error("invalid_json"); value(); space(); if (text[index] === "}") { index += 1; return; } if (text[index++] !== ",") throw new Error("invalid_json"); } } if (text[index] === "[") { index += 1; space(); if (text[index] === "]") { index += 1; return; } for (;;) { value(); space(); if (text[index] === "]") { index += 1; return; } if (text[index++] !== ",") throw new Error("invalid_json"); } } if (text[index] === '"') { string(); return; } const match = /^(?:true|false|null|-?(?:0|[1-9]\d*)(?:\.\d+)?(?:[eE][+-]?\d+)?)/u.exec(text.slice(index)); if (!match) throw new Error("invalid_json"); index += match[0].length; };
	value(); space(); if (index !== text.length) throw new Error("invalid_json");
}

function onceAsync(action: () => Promise<void>): () => Promise<void> { let pending: Promise<void> | undefined; return () => { pending ??= action().catch(() => undefined); return pending; }; }
function isJsonContentType(value: string | string[] | undefined): boolean { return typeof value === "string" && /^(?:application\/json)(?:\s*;|\s*$)/iu.test(value); }
