...
test("protocol negotiation selects the current implemented version", () => {
	assert.deepEqual(negotiateCealProtocol({ minimum: "1.4.0", maximum: "1.4.0" }), {
		schema_version: "ceal.protocol_negotiation.v1",
		ok: true,
		protocol_version: "1.4.0",
	});
	assert.equal(negotiateCealProtocol({ minimum: "0.9.0", maximum: "1.4.0" }).ok, true);
});
...
test("protocol negotiation fails closed on malformed or reversed ranges", () => {
	for (const range of [
		...
		{},
		"1.4.0",
		{ minimum: 1, maximum: "1.4.0" },
		{ minimum: "1", maximum: "1.4.0" },
		{ minimum: "1.4.0", maximum: "1.3.0-next" },
		{ minimum: "2.0.0", maximum: "1.4.0" },
