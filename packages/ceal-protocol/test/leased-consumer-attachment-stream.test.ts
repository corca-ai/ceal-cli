...
} satisfies ReturnType<typeof validateCealLeasedConsumerAttachmentStreamManifest>;
...
test("attachment stream protocol rejects result frames, path/provider leakage, and crossed lengths", () => {
	...
	assert.throws(() => validateCealLeasedConsumerAttachmentStreamManifest({ ...manifest, gateway_path: "/tmp/provider" }), (error) => hasCode(error, "invalid_manifest"));
	assert.throws(() => decodeCealLeasedConsumerAttachmentStreamFrameHeader({ schema_version: "ceal.result_materialization_frame.v1", kind: "chunk", slot: 0, chunk_index: 0, chunk_count: 1, bytes_base64: "YQ==" }), (error) => hasCode(error, "invalid_frame"));
	assert.throws(() => encodeCealLeasedConsumerAttachmentStreamRecord({ schema_version: CEAL_LEASED_CONSUMER_ATTACHMENT_STREAM_FRAME_SCHEMA, kind: "attachment", slot: 0, byte_length: 3 }, new Uint8Array([1])), (error) => hasCode(error, "attachment_length_mismatch"));
	assert.throws(() => decodeCealLeasedConsumerAttachmentStreamRecord(new Uint8Array([0, 0, 0, 1, 0, 0, 0, 0])), (error) => hasCode(error, "invalid_record_length"));
});
...
test("attachment stream protocol enforces the declared effective total and hard ceiling", () => {
	...
	assert.throws(() => validateCealLeasedConsumerAttachmentStreamManifest(tooSmall), (error) => hasCode(error, "invalid_manifest"));
	...
	}), (error) => hasCode(error, "invalid_limits"));
});
