...
export const CEAL_CLIENT_REFRESH_REQUEST_V2_SCHEMA = "ceal.client_refresh_request.v2" as const;
export const CEAL_CLIENT_REFRESH_RESULT_V2_SCHEMA = "ceal.client_refresh_result.v2" as const;
...
export interface CealClientRefreshRequestV2 {
	schema_version: typeof CEAL_CLIENT_REFRESH_REQUEST_V2_SCHEMA;
	refresh_token: string;
	refresh_attempt_ref: string;
	client: { name: "ceal"; version: string };
}

export interface CealClientRefreshResultV2 extends Omit<CealClientRefreshResult, "schema_version"> {
	schema_version: typeof CEAL_CLIENT_REFRESH_RESULT_V2_SCHEMA;
	refresh_attempt_ref: string;
	refresh_delivery: "initial" | "recovery";
}

export interface CealClientSessionFailure {
	schema_version: typeof CEAL_CLIENT_REFRESH_RESULT_SCHEMA | typeof CEAL_CLIENT_REFRESH_RESULT_V2_SCHEMA | typeof CEAL_CLIENT_REVOKE_RESULT_SCHEMA;
	ok: false;
	error: {
		code: "refresh_invalid" | "refresh_expired" | "refresh_replayed" | "refresh_revoked" | "refresh_recovery_unavailable";
		message: string;
		next_action: string;
	};
}
...
export type CealClientRefreshResponseV2 = CealClientRefreshResultV2 | CealClientSessionFailure;
...
export const CEAL_CLIENT_REFRESH_ATTEMPT_REF = /^ceal_refresh_attempt_[A-Za-z0-9_-]{43}$/u;
const VERSION = /^(0|[1-9][0-9]*)[.](0|[1-9][0-9]*)[.](0|[1-9][0-9]*)$/u;
const REFRESH_FAILURE_CODES = new Set(["refresh_invalid", "refresh_expired", "refresh_replayed", "refresh_revoked", "refresh_recovery_unavailable"]);
const REVOKE_FAILURE_CODES = new Set(["refresh_invalid", "refresh_expired", "refresh_replayed", "refresh_revoked"]);
...
export function decodeCealClientRefreshRequestV2(value: unknown): CealClientRefreshRequestV2 {
	const record = requireRecord(value);
	requireExactKeys(record, ["client", "refresh_attempt_ref", "refresh_token", "schema_version"]);
	const client = requireRecord(record.client);
	requireExactKeys(client, ["name", "version"]);
	if (record.schema_version !== CEAL_CLIENT_REFRESH_REQUEST_V2_SCHEMA
		|| typeof record.refresh_token !== "string" || !REFRESH_TOKEN.test(record.refresh_token)
		|| typeof record.refresh_attempt_ref !== "string" || !CEAL_CLIENT_REFRESH_ATTEMPT_REF.test(record.refresh_attempt_ref)
		|| client.name !== "ceal" || typeof client.version !== "string" || !VERSION.test(client.version)) invalid();
	return record as unknown as CealClientRefreshRequestV2;
}
...
export function decodeCealClientRefreshResponse(value: unknown): CealClientRefreshResponse {
	...
	if (record.ok === false) return decodeFailure(record, CEAL_CLIENT_REFRESH_RESULT_SCHEMA, REFRESH_FAILURE_CODES);
...
export function decodeCealClientRefreshResponseV2(value: unknown): CealClientRefreshResponseV2 {
	const record = requireRecord(value);
	if (record.schema_version !== CEAL_CLIENT_REFRESH_RESULT_V2_SCHEMA || typeof record.ok !== "boolean") invalid();
	if (record.ok === false) return decodeFailure(record, CEAL_CLIENT_REFRESH_RESULT_V2_SCHEMA, REFRESH_FAILURE_CODES);
	requireExactKeys(record, [
		"access_token", "client_ref", "expires_at", "instance_ref", "membership_ref", "ok", "profile_ref", "refresh_attempt_ref", "refresh_delivery", "refresh_token",
		"refresh_token_absolute_expires_at", "refresh_token_idle_expires_at", "registration_ref", "schema_version", "subject_ref",
	]);
	if (typeof record.refresh_attempt_ref !== "string" || !CEAL_CLIENT_REFRESH_ATTEMPT_REF.test(record.refresh_attempt_ref)
		|| (record.refresh_delivery !== "initial" && record.refresh_delivery !== "recovery")
		|| !validRefreshBinding(record) || !validRefreshTokens(record) || !validRefreshTimestamps(record)) invalid();
	return record as unknown as CealClientRefreshResultV2;
}
...
export function decodeCealClientRevokeResponse(value: unknown): CealClientRevokeResponse {
	...
	if (record.ok === false) return decodeFailure(record, CEAL_CLIENT_REVOKE_RESULT_SCHEMA, REVOKE_FAILURE_CODES);
...
function decodeFailure(
	...
	schema: typeof CEAL_CLIENT_REFRESH_RESULT_SCHEMA | typeof CEAL_CLIENT_REFRESH_RESULT_V2_SCHEMA | typeof CEAL_CLIENT_REVOKE_RESULT_SCHEMA,
	...
	if (record.schema_version !== schema || !failureCodes.has(String(error.code))
