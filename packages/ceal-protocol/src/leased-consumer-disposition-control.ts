...
} from "./leased-consumer-notification-control.ts";
import {
	decodeCealLeasedConsumerCapabilityReceipt,
	type CealLeasedConsumerCapabilityReceipt,
} from "./leased-consumer-capability-receipt.ts";
import { opaqueResultRef } from "./leased-consumer-opaque-refs.ts";

export * from "./leased-consumer-notification-control.ts";
export * from "./leased-consumer-capability-receipt.ts";
...
export type CealLeasedConsumerCapabilityReplayInput = {
	event_ref: string;
	lease_ref: string;
	lease_fence: number;
	result_ref: string;
};
export type CealLeasedConsumerDispositionControlRequest =
	| WithDispositionSchema<CealLeasedConsumerNotificationControlRequest, typeof CEAL_LEASED_CONSUMER_DISPOSITION_CONTROL_REQUEST_SCHEMA>
	| { schema_version: typeof CEAL_LEASED_CONSUMER_DISPOSITION_CONTROL_REQUEST_SCHEMA; operation: "result"; input: CealLeasedConsumerCapabilityReplayInput };
export type CealLeasedConsumerCapabilityDispositionResult =
	...
	| { status: "result_available"; result_ref: string; provider_outcome: "verified"; result_delivery: "offered" | "transport_lost" };
...
export type CealLeasedConsumerCapabilityReplayResult =
	| { status: "result"; result: CealLeasedConsumerCapabilityResult; receipt: CealLeasedConsumerCapabilityReceipt | null; replayed: true }
	| { status: "result_unavailable"; provider_outcome: "verified"; result_delivery: "unavailable" }
	| { status: "lease_lost" | "lease_expired" | "authentication_failed" };
export type CealLeasedConsumerDispositionControlResponse =
	...
	| { schema_version: typeof CEAL_LEASED_CONSUMER_DISPOSITION_CONTROL_RESPONSE_SCHEMA; operation: "result"; result: CealLeasedConsumerCapabilityReplayResult };
...
export function decodeCealLeasedConsumerDispositionControlRequest(value: unknown): CealLeasedConsumerDispositionControlRequest {
	...
	if (record.operation === "result") decodeReplayInput(record.input);
	else decodeCealLeasedConsumerNotificationControlRequest({ ...record, schema_version: CEAL_LEASED_CONSUMER_NOTIFICATION_CONTROL_REQUEST_SCHEMA });
...
export function decodeCealLeasedConsumerDispositionControlResponse(value: unknown): CealLeasedConsumerDispositionControlResponse {
	...
	else if (record.operation === "result") decodeReplayResult(record.result);
...
function decodeReplayInput(value: unknown): void {
	const input = requireRecord(value);
	requireExactKeys(input, ["event_ref", "lease_fence", "lease_ref", "result_ref"]);
	if (!safeRef(input.event_ref) || !safeRef(input.lease_ref) || !positive(input.lease_fence) || !opaqueResultRef(input.result_ref)) invalid();
}

function decodeReplayResult(value: unknown): void {
	const result = requireRecord(value);
	if (result.status === "result") {
		requireExactKeys(result, ["receipt", "replayed", "result", "status"]);
		if (result.replayed !== true || (result.receipt !== null && result.receipt === undefined)) invalid();
		...
		if (result.receipt !== null) decodeCealLeasedConsumerCapabilityReceipt(result.receipt);
		return;
	}
	if (result.status === "result_unavailable") {
		requireExactKeys(result, ["provider_outcome", "result_delivery", "status"]);
		if (result.provider_outcome !== "verified" || result.result_delivery !== "unavailable") invalid();
		return;
	}
	requireExactKeys(result, ["status"]);
	if (!["lease_lost", "lease_expired", "authentication_failed"].includes(String(result.status))) invalid();
}
...
function decodeCallResult(value: unknown): void {
	...
	if (result.status === "result_available") return decodeResultAvailable(result);
...
function decodeResultAvailable(result: Record<string, unknown>): void {
	requireExactKeys(result, ["provider_outcome", "result_delivery", "result_ref", "status"]);
	if (result.provider_outcome !== "verified" || !["offered", "transport_lost"].includes(String(result.result_delivery)) || !opaqueResultRef(result.result_ref)) invalid();
}
...
function safeRef(value: unknown): value is string { return typeof value === "string" && /^[A-Za-z0-9][A-Za-z0-9._:-]{0,255}$/u.test(value); }
