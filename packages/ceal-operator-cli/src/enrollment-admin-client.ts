...
export async function createCealEnrollment(
	...
	const { endpoint, fetchFn, timeoutMs } = validateRequestConfiguration(input, options);
	const controller = new AbortController();
	const timer = setTimeout(() => controller.abort(), timeoutMs);
	try {
		return await performEnrollmentRequest(input, endpoint, fetchFn, controller.signal);
	} catch (error) {
...
function validateRequestConfiguration(
	input: CealEnrollmentCreateInput,
	options: { fetchFn?: typeof globalThis.fetch; timeoutMs?: number },
): { endpoint: URL; fetchFn: typeof globalThis.fetch; timeoutMs: number } {
...
async function performEnrollmentRequest(
	input: CealEnrollmentCreateInput,
	endpoint: URL,
	...
	return decodeResult(parseJson(bytes));
}
